...
We measure the error using the **Mean Squared Error (MSE)** cost function. A convenient form (with a factor \( \frac{1}{2} \) to simplify derivatives) is:
...
Sum of errors:  
\[
\sum_{i=1}^{5} (h_{\mathbf{b}}(x_i)-y_i) = -50 -55 -65 -70 -80 = -320
\]
...
1. Define the cost function \( J(b_0,b_1) = \frac{1}{2n}\sum (h_{\mathbf{b}}(x_i)-y_i)^2 \).
2. Compute the gradients:
   \[
   \frac{\partial J}{\partial b_0} = \frac{1}{n}\sum (h_{\mathbf{b}}(x_i)-y_i),\quad
   \frac{\partial J}{\partial b_1} = \frac{1}{n}\sum (h_{\mathbf{b}}(x_i)-y_i)x_i
   \]
3. Update parameters simultaneously:
   \[
   b_0 := b_0 - \alpha \frac{\partial J}{\partial b_0},\quad
...
- The analytical OLS solution is \( \mathbf{b} = (X^TX)^{-1}X^T\mathbf{y} \), but gradient descent scales better to very large datasets.
